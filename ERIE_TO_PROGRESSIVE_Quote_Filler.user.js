...
(function () {
  ...
  async function setDisclosureProvidedLast(result, value) {
    const wanted = String(value || '').trim().toUpperCase();
    if (!wanted) return false;

    // Progressive may redraw the Named Insured view after other fields settle.
    // Always re-fetch the select on every attempt because Angular can replace it.
    const maxAttempts = 4;

    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      const el = getElement(IDS.disclosureProvided);

      if (!el || el.tagName !== 'SELECT') {
        result.steps.push({
          phase: 'final_disclosure',
          id: IDS.disclosureProvided,
          value: wanted,
          ok: false,
          attempt: attempt,
          error: 'Disclosure select not found'
        });
        await sleep(500);
        continue;
      }
      ...
      const ok = setSelect(IDS.disclosureProvided, wanted);
      ...
      // Give Progressive enough time to perform any final view-model redraw.
      await sleep(attempt === 1 ? 900 : 700);

      if (valuesEqual(readFieldValue(IDS.disclosureProvided), wanted)) {
        return true;
      }
    }

    result.warnings.push('Disclosure selection did not remain set after Progressive redraws.');
    return false;
  }

  async function setZipCodeLast(result, value) {
    const wanted = String(value || '').replace(/\D+/g, '').slice(0, 5);
    if (!wanted) return false;

    // Progressive can rebuild the Named Insured Angular view after State,
    // Recently Moved, and Disclosure updates. ZIP is especially vulnerable
    // because the replacement input comes back from the server with a blank
    // ZipCode value. Re-fetch and commit ZIP only after every other field has
    // finished, then verify that it survives the redraw.
    const maxAttempts = 5;

    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      const el = getElement(IDS.zip);
      ...
      // Angular listens to the input event for the model value. Follow with
      // change and a real blur so Progressive commits the field before any
      // final redraw.
      el.dispatchEvent(new Event('input', { bubbles: true }));
      el.dispatchEvent(new Event('change', { bubbles: true }));
      ...
      const firstCheck = valuesEqual(readFieldValue(IDS.zip), wanted);
      if (firstCheck) {
        // Give the Progressive view model one more redraw window. If ZIP is
        // still present after this delay, it has actually stuck.
        await sleep(1000);
      }

      const finalCheck = valuesEqual(readFieldValue(IDS.zip), wanted);
    ...
    result.warnings.push('ZIP Code did not remain set after Progressive redraws.');
    return false;
  }
  ...
  async function fillProgressiveNamedInsuredFromErie(payload, options) {
    ...
    // Disclosure is handled near the end because Progressive can redraw the
    // Named Insured view after this selection changes.
    await waitForDomStability(STABLE_WATCH_IDS, 1400, 9000, 150);
    await setDisclosureProvidedLast(result, data.disclosureProvided);

    // ZIP must be the final committed field. Progressive's Angular view can
    // replace the ZIP input during the late redraw above and restore the
    // server-side blank value even though the ZIP briefly appeared on screen.
    await waitForDomStability(STABLE_WATCH_IDS, 900, 7000, 150);
    await setZipCodeLast(result, data.zip);
...
// ============================================================
// PROGRESSIVE - EMBEDDED FILL PAGE BUTTON
// =======================================
(function () {
    'use strict';

    const ROOT = (typeof unsafeWindow !== 'undefined') ? unsafeWindow : window;
    const BUTTON_ID = 'mci-progressive-fill-page';
    const STYLE_ID = 'mci-progressive-fill-page-style';
    const TOAST_ID = 'mci-progressive-fill-page-toast';
    ...
    function toast(message, ms) {
        ...
        clearTimeout(el._mciHideTimer);
        el._mciHideTimer = setTimeout(function () {
            el.style.display = 'none';
        }, ms || 2200);
    }

    function resolveFunction(name) {
        const roots = [ROOT, window];
        ...
        for (let i = 0; i < roots.length; i += 1) {
            try {
                if (roots[i] && typeof roots[i][name] === 'function') {
                    return { root: roots[i], fn: roots[i][name] };
                }
            } catch (_) {}
        }
    ...
    function detectCurrentPage() {
        ...
        const householdMain = document.querySelector('main[aria-labelledby="HouseholdMembers"]');
        const householdHeading = Array.from(document.querySelectorAll('h1')).some(function (el) {
            return String(el.textContent || '').trim() === 'Household Members';
        });

        if (householdMain || householdHeading) {
            return {
    ...
    function runCurrentPage() {
        ...
        const target = resolveFunction(page.fnName);
        if (!target) {
            toast('Progressive filler not found for ' + page.name + '.');
            return;
        }

        try {
            const result = target.fn.apply(target.root, page.args);

            if (result && typeof result.then === 'function') {
                result.catch(function (error) {
                    console.warn('[MCI Progressive Fill Page] Error:', error);
                    toast('Progressive Fill Page failed - see console.');
                });
            }
        } catch (error) {
            console.warn('[MCI Progressive Fill Page] Error:', error);
    ...
    function ensureStyle() {
        ...
        style.textContent = `
            #${BUTTON_ID} {
                ...
                border-radius: 5px;
                background: linear-gradient(180deg, #18a4d8 0%, #0787bd 55%, #006d9c 100%);
                color: #fff;
                box-shadow: 0 2px 7px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.22);
                cursor: pointer;
            ...
            #${BUTTON_ID}:hover,
            #${BUTTON_ID}:focus {
                background: linear-gradient(180deg, #2bb5e8 0%, #1297cd 55%, #087caf 100%);
                box-shadow: 0 3px 10px rgba(0,0,0,.35), inset 0 1px 0 rgba(255,255,255,.28);
                outline: none;
            }
    ...
    function mountButton() {
        ...
        if (!header || !document.body) return false;
        ...
        button.id = BUTTON_ID;
        button.type = 'button';
        button.textContent = 'Fill Page';
        button.setAttribute('aria-label', 'Fill current Progressive quote page');
        ...
        // Important: mount outside Progressive's Angular header DOM.
        // The header has tightly coupled layout rules; adding a child inside it
        // can change or hide Progressive's native header controls.
    ...
    ROOT.runProgressiveCurrentPage = runCurrentPage;
    try {
        window.runProgressiveCurrentPage = runCurrentPage;
    } catch (_) {}
