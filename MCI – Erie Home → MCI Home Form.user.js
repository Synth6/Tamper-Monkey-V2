...
// @grant        GM_getValue
// @grant        unsafeWindow
// @updateURL  https://raw.githubusercontent.com/Synth6/Tamper-Monkey-V2/main/MCI%20–%20Erie%20Home%20→%20MCI%20Home%20Form.user.js
// @downloadURL  https://raw.githubusercontent.com/Synth6/Tamper-Monkey-V2/main/MCI%20–%20Erie%20Home%20→%20MCI%20Home%20Form.user.js
// ==/UserScript==
...
(function () {
  ...
  function getSelectedText(selector) {
    ...
    console.log('[MCI Home] Selected text for', selector, 'â†’', value);
    return value;
  }
  ...
  function getValue(selector) {
    ...
    console.log('[MCI Home] Value for', selector, 'â†’', value);
    return value;
  }
  ...
  function getCurrentHomeInsurerText() {
    ...
    console.log('[MCI Home] CurrentHomeInsurer â†’ value =', value, ', text =', text);
  ...
  function getText(selector) {
    ...
    console.log('[MCI Home] Text for', selector, 'â†’', value);
    return value;
  }
  ...
  function getCounty() {
    ...
    if (sel) {
      ...
      console.log('[MCI Home] County from #selMailingCountyList â†’', val);
      return val;
    }
    ...
    if (span) {
      ...
      console.log('[MCI Home] County from bold span â†’', value);
      return value;
    }
  ...
  function getNamedInsuredEmail() {
    ...
    console.log('[MCI Home] Named insured email â†’', email);
    return email;
  }

  // Generic helper for Erieâ€™s <obscured-text-with-toggle> custom elements
  function grabObscuredValue(paramSubstring) {
    ...
    console.log(`[MCI Home] Obscured value for ${paramSubstring} â†’`, txt);
    return txt;
  }
  ...
  async function getFullDateOfBirth(index = 0, allowReveal = true) {
    ...
    if (allowReveal && txt.indexOf('*') !== -1 && toggle) {
      console.log('[MCI Home] DOB appears masked at index', index, 'â€“ clicking eye iconâ€¦');
      toggle.click();
      await wait(300);
      txt = (span.innerText || span.textContent || '').trim();
      console.log('[MCI Home] DOB after reveal at index', index, 'â†’', txt);
    } else {
      console.log('[MCI Home] DOB at index', index, 'â†’', txt);
    }
  ...
  function normalizeDateToISO(text) {
    ...
    const iso = `${yy}-${mm}-${dd}`;
    console.log('[MCI Home] Normalized date', text, 'â†’', iso);
    return iso;
  }

  // ===== STEP 1 â€“ CUSTOMER TAB =====
  async function collectCustomerData() {
    console.log('[MCI Home] Collecting Customer-tab dataâ€¦');
  ...
  async function handleCustomerClick() {
    try {
      console.log('[MCI Home] Step 1 â€“ collecting and storing Customer dataâ€¦');
  ...
  // ===== STEP 2 â€“ DWELLING TAB =====

  function collectDwellingData() {
    console.log('[MCI Home] Collecting Dwelling-tab dataâ€¦');
    ...
    if (pcSel) {
      ...
      console.log('[MCI Home] ProtectionClass / Fire District â†’', pcVal);
    } else {
  ...
  async function handleDwelling(autoMode = false) {
    try {
      console.log('[MCI Home] Step 2 â€“ Dwelling step startingâ€¦');
      const baseData = GM_getValue(STORAGE_KEY, null);
      if (!baseData) {
        console.warn('[MCI Home] No stored Customer data â€“ aborting Dwelling step.');
        GM_setValue(FLOW_KEY, 'idle');
        if (!autoMode) {
      ...
      if (!clicked && autoMode) {
        ...
        if (idx > -1) {
          ...
          console.log('[MCI Home] Fallback â€“ navigating to Coverages via URL:', targetUrl);
          window.location.href = targetUrl;
        } else {
  ...
  // ===== STEP 3 â€“ COVERAGES TAB =====

  function collectCoveragesData() {
    console.log('[MCI Home] Collecting Coverages-tab dataâ€¦');
  ...
  // Ensure weâ€™re on the Home sub-tab before collecting Coverages
  async function ensureHomeCoveragesPanel() {
    ...
    console.log('[MCI Home] Clicking Home coverages sub-tabâ€¦');
    homeLink.click();

    // Wait for Erieâ€™s JS to swap the panel in
  ...
  async function handleCoverages(autoMode = false) {
    try {
      console.log('[MCI Home] Step 3 â€“ Coverages/export step startingâ€¦');
      const baseData = GM_getValue(STORAGE_KEY, null);
      if (!baseData) {
        console.warn('[MCI Home] No stored data â€“ aborting Coverages step.');
        GM_setValue(FLOW_KEY, 'idle');
        if (!autoMode) {
  ...
  function init() {
    ...
    if (href.includes('/Customer')) {
      ...
      console.log('[MCI Home] On Customer page â€“ waiting for MCI Toolbox trigger.');
      ...
      if (flow === 'after_customer') {
        ...
        console.log('[MCI Home] On Dwelling page but flow state is', flow, 'â€“ not auto-running.');
      }

    } else if (href.includes('/Coverages')) {
      if (flow === 'after_dwelling') {
        console.log('[MCI Home] Auto-mode: on Coverages page â€“ ensuring Home tab, then exporting.');
        handleCoverages(true);
      } else {
        console.log('[MCI Home] On Coverages page but flow state is', flow, 'â€“ not auto-running.');
      }
    }
  }
