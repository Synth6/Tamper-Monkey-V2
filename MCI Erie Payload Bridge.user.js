...
(function () {
  ...
  function payloadDisplayName(payload) {
    const p = payload || {};
    const customer = p.customer || {};
    return text(customer.fullName) ||
      [text(customer.firstName), text(customer.middleName), text(customer.lastName)]
        .filter(Boolean).join(' ') ||
      'MCI customer';
  }

  function looksLikeMciQuotePayload(payload) {
  ...
  async function importMciPayloadFromClipboard() {
    let raw = '';
    ...
    if (!text(raw)) {
      raw = window.prompt(
        'Paste the Customer Search quote data here, then click OK.',
        ''
      ) || '';
    }
    ...
    if (!text(raw)) {
      if (clipboardError) {
        console.warn(LOG, 'clipboard read unavailable', clipboardError);
      }
    ...
    try {
      ...
      window.alert('The clipboard does not contain valid MCI quote JSON.');
      return { ok: false, reason: 'invalid_json', error: String(e) };
    }

    if (!looksLikeMciQuotePayload(payload)) {
      window.alert('The clipboard JSON does not look like MCI Customer Search quote data.');
    ...
    if (!text(payload.meta.updatedAt)) {
      payload.meta.updatedAt = new Date().toISOString();
    }
    if (!text(payload.meta.createdAt)) {
      payload.meta.createdAt = payload.meta.updatedAt;
    }

    const ok = setMciSharedPayload(payload);
    if (!ok) {
      window.alert('MCI quote data could not be loaded into the shared payload.');
      return { ok: false, reason: 'save_failed' };
    }

    const name = payloadDisplayName(payload);
    console.log(LOG, 'Customer Search payload loaded', {
      name: name,
      summary: payloadSummary(payload)
    });
  ...
  root.importMciPayloadFromClipboard = importMciPayloadFromClipboard;
