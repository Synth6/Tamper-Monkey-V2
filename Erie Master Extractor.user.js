...
(function () {
  ...
  const Norm = {
    ...
    enrichVehicleFromDom(vehicle) {
      ...
      const vin = U.inputValue('#VIN, #VehicleIdentificationNumber, #Vin, input[name="VehicleIdentificationNumber"], input[name="VIN"]');
      if (vin) out.vin = vin;
  ...
  const Extractors = {
    ...
    vehicles(ctx) {
      ...
      const fullVin = U.inputValue('#VIN, #VehicleIdentificationNumber, #Vin, input[name="VehicleIdentificationNumber"], input[name="VIN"]');
      if (fullVin) {
        let target = null;
  ...
  const Actions = {
    ...
    readOpenVehicleFullVin() {
      ...
      const vinEl = U.safeQuery('#VIN');
      addCandidate(vinEl && vinEl.value);
      addCandidate(vinEl && vinEl.getAttribute('value'));

      try {
        const root = typeof unsafeWindow !== 'undefined' ? unsafeWindow : window;

        if (root.ko && vinEl) {
          const ctx = root.ko.contextFor(vinEl);
          const koRoot = ctx && ctx.$root;
          const koData = ctx && ctx.$data;

          if (koRoot && koRoot.VehicleFormContainer) {
            const container = unwrap(koRoot.VehicleFormContainer);
            const form = container && unwrap(container.VehicleForm);
            addCandidate(form && unwrap(form.VIN));
          }

          if (koData && koData.VehicleFormContainer) {
            const container2 = unwrap(koData.VehicleFormContainer);
            const form2 = container2 && unwrap(container2.VehicleForm);
            addCandidate(form2 && unwrap(form2.VIN));
          }
        }

        if (root.plw && root.plw.vehicles && root.plw.vehicles.viewModel) {
          const vm = root.plw.vehicles.viewModel;
          const container3 = unwrap(vm.VehicleFormContainer);
          const form3 = container3 && unwrap(container3.VehicleForm);
          addCandidate(form3 && unwrap(form3.VIN));
        }
      } catch (e) {
        U.debug('VIN read from Knockout failed:', e);
      }

      for (let i = 0; i < candidates.length; i++) {
        const vin = U.cleanString(candidates[i]).toUpperCase();
        if (vin && vin.length === 17 && !U.looksMasked(vin)) {
          return vin;
        }
      }
    ...
    async waitForVinChange(previousVin, timeout) {
      ...
      while (Date.now() - start < timeout) {
        const koVin = Actions.readOpenVehicleFullVin();
        if (koVin) {
          return koVin;
        }
        ...
        const el = U.safeQuery('#VIN') ||
          ...
          U.safeQuery('input[id*="VIN"]') ||
          U.safeQuery('input[name*="VIN"]');

        const val = U.cleanString(el && el.value || '');

        if (val && val.length === 17 && !U.looksMasked(val)) {
          return val;
        }
      ...
      try {
        ...
        for (let i = 0; i < vehicleLinks.length; i++) {
          ...
          await U.waitFor(function () {
            return Actions.readOpenVehicleFullVin();
          }, 8000, 200);

          await U.delay(300);

          const newVin = Actions.readOpenVehicleFullVin() || await Actions.waitForVinChange('', 8000);
          if (!newVin) {
            state.skipped++;
            U.debug('VIN harvest skipped; no full VIN found for vehicle index', i);
            UI.toast('VIN ' + (i + 1) + '/' + state.total + ' skipped');
            UI.refresh();
            await U.delay(250);
            continue;
    ...
    showSummary() {
      try {
        Actions.collectCurrentPage({ silent: true });
      } catch (e) {
        U.debug('Collect before summary failed:', e);
      }
